...
class AIAssistantService:
    ...
    async def triage_image(
        ...
        """Triage an uploaded image using Gemini Vision with per-step latency instrumentation."""
        ...
        with PipelineTimer("triage_image") as timer:
            ...
            with timer.step("gemini_vision"):
                try:
                    ...
                    vision_failed = False
                    vision_mock = bool(result.raw_response and result.raw_response.get("mock"))
                except Exception as e:
                    vision_failed = True
                    vision_error = str(e)

            if vision_failed:
                timing_payload = timer.report().as_dict()
            ...
            # Step 2: Category mapping + priority scoring
            with timer.step("category_priority_mapping"):
